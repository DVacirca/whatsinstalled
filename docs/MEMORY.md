...
- **Refactor**: extracted the ranking out of `(*model).search` into a pure, UI/DB/network-free `search.Rank(queryVec, query, pkgs, opts) []Result` (`internal/search/rank.go`); the TUI and the harness now score with identical code. `search.Options{KeywordWeight, Threshold, TopK}`; `DefaultOptions` = {0.0, 0.05, 50} (KeywordWeight zeroed after eval showed it hurts MRR). Query expansion (`nlp.ExpandQuery`) stays at the caller (it changes the embedded text). `(*model).search` rewired to call it; the no-embeddings `SearchText` fallback stayed. Removed now-unused `sort` import from `dashboard.go`.
